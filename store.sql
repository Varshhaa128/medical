...
--
DELIMITER $$
CREATE TRIGGER `check_price_before_insert` BEFORE INSERT ON `inventeries` FOR EACH ROW BEGIN
    IF NEW.price > 500 THEN
        SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT = 'Price per unit should not be greater than 500';
    END IF;
END
$$
DELIMITER ;
